...
export default class Uploader {
   ...
   */
  constructor({ config, onUpload, onUploadPoster, onError }) {
    this.config = config;
   ...
   */
  uploadSelectedPoster({ handlePoster }) {
    const preparePreview = function (file) {
      const reader = new FileReader();
    ...
    const upload = ajax.selectFiles({ accept: 'image/*' }).then((files) => {
      preparePreview(files[0]);
